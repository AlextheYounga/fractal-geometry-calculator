...
def writeCSV(fractal_results, refScales, resultsfile, append):
    ...
    hurstExponents = extract_scaled_data(results_scales, fractal_results['regressionResults'], 'hurstExponent')
    fractalDimensions = extract_scaled_data(results_scales, fractal_results['regressionResults'], 'fractalDimension')
    rSquared = extract_scaled_data(results_scales, fractal_results['regressionResults'], 'r-squared')
    pValues = extract_scaled_data(results_scales, fractal_results['regressionResults'], 'p-value')
    standardErrors = extract_scaled_data(results_scales, fractal_results['regressionResults'], 'standardError')
...
def exportFractal(fractal_results, refScales):
    output_file = 'fractalgeometrycalculator/exports/fractal_results.csv'
